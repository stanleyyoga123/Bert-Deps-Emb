...
class BertDependencyEmbeddings(BertEmbeddings):
    def __init__(self, config, word_emb_dim=300, dep_emb_dim=300, maxlen=512):
        super(BertDependencyEmbeddings, self).__init__(config)
        self.dep_embeddings = None
        self.word_dep_embeddings = None
        self.fc = nn.Linear(dep_emb_dim + word_emb_dim, config.hidden_size)
        self.maxlen = maxlen
        ...
        for i, batch_deps in enumerate(deps_ids):
            ...
            for j, deps_id in enumerate(batch_deps):
                ...
                if sum_:
                    ...
                    temp.append(taken_emb / sum_)
                else:
                    temp.append(torch.zeros_like(word_dep_embeddings[i][j]))
            concat_mat.append(torch.stack(temp))
        concat_mat = torch.stack(concat_mat)
        input_fc = torch.cat([word_dep_embeddings, concat_mat], dim=-1)
        dep_pad_loc = self.maxlen - (dep_input_ids == self.word_pad_id).sum(axis=1)
        
        # FC for Dependency Embeddings
        fc_dep = self.fc(input_fc.float())
...
class BertDependencyLocalConcatModel(BertModel):
    def __init__(self, config, num_labels=2, output_attentions=False, keep_multihead_output=False):
        super(BertDependencyLocalConcatModel, self).__init__(config, output_attentions=False, keep_multihead_output=False)

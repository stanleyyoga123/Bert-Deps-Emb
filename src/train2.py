...
if __name__ == "__main__":
    ...
    if ds == "cola":
        loader_train = DataLoader(CoLADepsDataset("train", word_tokenizers, deps_tokenizer, conv=True), batch_size=16, shuffle=True)
        loader_dev = DataLoader(CoLADepsDataset("dev", word_tokenizers, deps_tokenizer, conv=True), batch_size=16, shuffle=True)
        loader_test = DataLoader(CoLADepsDataset("test", word_tokenizers, deps_tokenizer, conv=True), batch_size=16, shuffle=True)
    elif ds == "sst":
        loader_train = DataLoader(SSTDepsDataset("train", word_tokenizers, deps_tokenizer, conv=True), batch_size=16, shuffle=True)
        loader_dev = DataLoader(SSTDepsDataset("dev", word_tokenizers, deps_tokenizer, conv=True), batch_size=16, shuffle=True)
        loader_test = DataLoader(SSTDepsDataset("test", word_tokenizers, deps_tokenizer, conv=True), batch_size=16, shuffle=True)
    ...
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(loader_train):
            word_ids, deps_ids, label = batch[0].to(device), batch[1].to(device), batch[2].type(torch.LongTensor).to(device)
            logits = model(word_ids, deps_ids)
        ...
        if f1_test > best_test:
            ...
            torch.save(to_save, os.path.join("save", f"{name}-{ds}-model-best-test-{epoch}"))
            best_test = f1_test
        ...
        if f1_dev > best_dev:
            ...
            torch.save(to_save, os.path.join("save", f"{name}-{ds}-model-best-dev-{epoch}"))
            best_dev = f1_dev
